...
## Application 1: finetune to predict 360 angles of a jigsaw piece

The training data is generated by generate_data_jigsaw_game.py by rotate original images of the jigsaw game piece and label automatically. The training is done in finetune_jigsaw_game.py. The finetuned model can predict the angle quite well.

## Application 2: finetune to predict trash classification

We use the data from trashnet to finetune Alexnet to predict 6 classes of garbage by finetune_trash.py. The trash data is converted to tfrecord files by generate_tfrecords.py.

The 6 classes are "carboard","glass", "metal","paper","plastic", "trash". The data is split into 80% training data and 20% test data. The test accuracy is between 0.7 ~0.8.
