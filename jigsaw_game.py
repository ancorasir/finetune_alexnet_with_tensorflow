...
def generate_data():
    ...
    for i in range(6):
        ...
        I_0.show()
        model_name = images2[i][16:-4]
        for j in range(1800):
            It = I_0.rotate(-j,expand=1,fillcolor=(33,128,37))
            du = np.random.randint(-20, 20) #left/right (i.e. 5/-5)
            dv = np.random.randint(-20, 20) #up/down (i.e. 5/-5)
            It = It.transform(It.size, Image.AFFINE, (1, 0, du, 0, 1, dv),fillcolor=(33,128,37))
            It.save('./train_data_real/image_%s_%s.png'%(model_name,j))
